...
function Room(props) {
  ...
  const handleSearchListRoom = async () => {
    // document.getElementById(storeId).setAttribute("class", "text-primary");
    // if (data == 0) {
    //   let res = await axios.get(apiUrl + `Rooms`);
    //   setState({
    ...
    // } else {
    let res;
    if (currentBlock === 0) res = await axios.get(apiUrl + `Rooms`);
    else res = await axios.get(apiUrl + `Rooms/${currentBlock}/Block`);
    setState({
      ListRooms: res.data ? res.data : [],
    });
    // setStoreId(5);
