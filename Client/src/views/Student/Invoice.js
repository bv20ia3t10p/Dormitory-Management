...
function Invoice(props) {
  ...
  useEffect(() => {
    async function fetchMyAPI() {
      let studentId = await axios.get(url + `api/Student/${id}/accountId`);
      let data = await axios.get(
        url + `api/RegisterRoom/${studentId.data.id}/student`
      );
  ...
  const PaymentRoom = async (item) => {
    ...
    try {
      let res = await axios.put(url + `api/RegisterRoom/${item.id}`, {
          ...
          {Invoice &&
            ...
            Invoice.map((item, index) => {
              return (
                <>
                  <div
                    class=" mb-2 border border-primary w-75 shadow"
                    style={{ "margin-left": "10%" }}
                  >
                    <div class="bg-primary text-white p-1">
                      <i
                        class="fa fa-table ml-2 text-white"
                        aria-hidden="true"
                      ></i>{" "}
                      Số biên lai: {item.id}
                    </div>
                    ...
                    <div class="ml-2 p-1">
                      Năm học:{" "}
                      {moment(item.dateBegin).format("YYYY") +
                        " - " +
                    ...
                    <table class=" ml-2 mr-2 border" style={{ width: "97%" }}>
                      ...
                      <tr>
                        <td class="p-1" style={{ "font-size": "17px" }}>
                          Phí lưu trú phòng phục vụ sinh viên từ ngày{" "}
                        ...
                        <td class="text-left">
                          {item.domitoryFeeStatus ? (
                            ...
                            <input
                              ...
                              value="Thanh toán"
                              onClick={() => handlePayFeeRoom(item)}
                            />
                          )}
                        </td>
                        <td class="text-right pr-5 pb-3 pt-3">
                          <span class="font-weight-bold text-danger">
                            {" "}
                            Tổng cộng:{" "}
                          </span>{" "}
                          <span>{item.domitoryFee}</span>
                        </td>
                      </tr>
                    </table>
                  </div>
